...
public class ServerThread extends Thread {
	...
	private DiningKeySet key;

	public ServerThread(SharedServerInfo sharedInfo,
			ClientSocketInfo clientConnection) {
	...
	public void run() {
		try {
			...
			while (true) {
				...
				// Wait until all clients have been send the collection of
				// outputs.
	...
	private void sendResults() throws IOException {
		...
		ArrayList<Message> results = new ArrayList<Message>(sharedInfo
	...
	private void abort() {
		...
		try {
			...
			sharedInfo.addOutput(new Message("" + key.sum()));
